...
describe( 'PanelColor', () => {
	...
	it( 'should have color when provided', () => {
		...
		expect( wrapper.prop( 'title' ) ).toContainEqual(
			<span className="components-panel__color-area" aria-label="red" key="color" style={ { background: 'red' } } />
		);
	} );
	...
	it( 'should use color name in area label if provided', () => {
		...
		expect( wrapper.prop( 'title' ) ).toContainEqual(
			<span className="components-panel__color-area" aria-label="red" key="color" style={ { background: '#f00' } } />
		);
	} );
} );
